...
import { commitLiveApi } from './config'
...
export default class RemoteFileSearchView {
  ...
  constructor(serializedState) {
    this.selectList = new SelectList({
      ...
      didConfirmSelection: (item) => {
        ...
        atom.commands.dispatch(atom.views.getView(atom.workspace), 'commit-live:re-render-welcome')
        this.modalPanel.hide();
      },
    });
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'commit-live:project-search': () => this.toggle()
    }));
  }
  ...
  fetchFileList() {
    ...
    const getAllProjectApi = `${commitLiveApi}/entity/projects/course/${courseId}`;
    fetch(getAllProjectApi, {
      headers,
    }).then((response) => {
      if (response.data && response.data.length) {
        this.updateList(response.data
          .filter(item => item.userStatus !== 'locked')
          // .map(item => item.name)
        , '');
      } else {
        this.updateList([{name:'Project 1'}, {name:'Project 2'}], 'No Projects Found!');
      }
    }).catch((err) => {
      this.updateList(['Project 1', 'Project 2'], 'No Projects Found!');
