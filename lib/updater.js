...
export default {
  ...
  _fetchLatestVersionData() {
    ...
    console.log('packageRepoName', packageRepoName);
    atom.notifications.addSuccess(`Commit Live IDE: packageRepoName! - ${packageRepoName}`);
    return fetch(`https://api.github.com/repos${packageRepoName}tags`).then(tagList => {
        this.latestVersionData = null;
        if (tagList && tagList.length && tagList[0]) {
          const {name} = tagList[0];
          console.log('===Commit Live IDE - latestVersion===');
          console.log(name);
          console.log('===Commit Live IDE - latestVersion===');
          this.latestVersionData = {
            version: name.replace('v', ''),
            detail: 'We have some updates for you!'
          };
        }
  ...
  _addUpdateNotification(detail) {
    this.updateNotification =
      atom.notifications.addInfo('Commit Live IDE: update available!', {
        detail,
        description: 'There have been few updates to the IDE!',
        dismissable: true,
        buttons: [{
