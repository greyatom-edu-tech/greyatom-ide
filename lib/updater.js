...
export default {
  autoCheck() {
    ...
    this._fetchLatestVersionData().then(({name, version, detail}) => {
      this._setCheckDate();

      if (this._shouldUpdate(version)) { this._addUpdateNotification(name, detail) }
    });
  },
  ...
  checkForUpdate() {
    this._fetchLatestVersionData().then(({name, version, detail}) => {
      ...
      if (this._shouldUpdate(version)) {
        this._addUpdateNotification(name, detail);
      }/* else {
  ...
  didRestartAfterUpdate() {
    var log = localStorage.delete('updateLog');
    var target = localStorage.delete('targetedUpdateVersion');
  ...
  _fetchLatestVersionData() {
    const {pkgRepository} = require('../package.json');
    const packageRepoName = pkgRepository.replace('https://github.com', '');
    const endpoint = `https://api.github.com/repos${packageRepoName}releases/latest`;
    return fetch(endpoint).then(latestRelease => {
      this.latestVersionData = null;
      if (latestRelease && latestRelease.tag_name) {
        const {tag_name, body, name} = latestRelease;
        console.log('===Commit Live IDE - latest-version===');
        console.log(tag_name);
        console.log('===Commit Live IDE - latest-version===');
        this.latestVersionData = {
  ...
  _shouldRollback() {
    var rollback = parseInt(localStorage.get('commit-live-ide:shouldRollback'));
  ...
  _addUpdateNotification(name, detail) {
    this.updateNotification =
      atom.notifications.addInfo('Commit Live IDE: Update Available!', {
        detail: 'Version Name: ' + name,
        description: detail,
        dismissable: true,
        buttons: [{
  ...
  _addUpToDateNotification() {
    atom.notifications.addSuccess('Commit Live IDE: up-to-date!');
  },
  ...
  _updateFailed(detail) {
    ...
    if (detail != null) {
      description = 'Please include this information when contacting the Commit Live support team about the issue.';
      buttons.push({
        text: 'Copy this log',
    ...
    this.updateNotification =
      atom.notifications.addWarning('Commit Live IDE: update failed!', {detail, description, buttons, dismissable: true});
  },

  _updateSucceeded() {
    atom.notifications.addSuccess('Commit Live IDE: update successful!');
  }
};
